...
interface ElkLayoutOptions {
  ...
  nodeSpacing?: string | number;
  layerSpacing?: string | number;
  grid?: number;
  tidyLinear?: boolean;
}

type GetNodeInternals = ((nodeId: string) => NodeInternal | null) | null;

// ids de puertos únicos en todo el grafo
const portId = (nodeId: string, handleId: string): string => `${nodeId}:${handleId}`;

const getNodeDimensions = (node: ReactFlowNode, getNodeInternals: GetNodeInternals = null): { width: number; height: number } => {
...
const makePort = (nodeId: string, handleId: string, side: string, index: number = 0) => ({
...
const getNodePorts = (node: ReactFlowNode) => {
  if (node.type === 'trigger' || node.type?.includes('trigger')) {
...
export const convertToElkGraph = (
  nodes: ReactFlowNode[],
  edges: ReactFlowEdge[],
  getNodeInternals: GetNodeInternals = null,
...
const snap = (v: number, grid: number = 20): number => Math.round(v / grid) * grid;

const analyzeLinearPath = (nodes: ReactFlowNode[], edges: ReactFlowEdge[]): { isLinear: boolean; order: string[] } => {
  const indeg = new Map(nodes.map((n) => [n.id, 0]));
  const outdeg = new Map(nodes.map((n) => [n.id, 0]));
  const next = new Map<string, string>(); // source -> target
...
const tidyLinear = (
  ...
  getNodeDimensions: (node: ReactFlowNode, getNodeInternals: GetNodeInternals) => { width: number; height: number }
): ReactFlowNode[] => {
...
export const applyElkLayout = async (
  ...
  try {
    ...
    const byId = new Map((layoutedGraph.children || []).map((n: { id: string | number; x?: number; y?: number }) => [String(n.id), n]));
