...
export function validateNode(node: unknown): ValidationResult {
  ...
  if (!baseValidation.success) {
    return { valid: false, errors: baseValidation.error.issues.map((e: { message?: string }) => e.message || JSON.stringify(e)) };
  }

  const typedNode = baseValidation.data;
  ...
  const configValidation = configSchema.safeParse(typedNode.config);
  if (!configValidation.success) {
    return { valid: false, errors: configValidation.error.issues.map((e: { message?: string }) => e.message || JSON.stringify(e)) };
  }
...
export function validateGraph(graph: unknown): ValidationResult {
  ...
  if (!Array.isArray(graphObj.nodes)) {
    graphObj.nodes = graphObj.nodes || [];
  }
  if (!Array.isArray(graphObj.edges)) {
    graphObj.edges = graphObj.edges || [];
  }

  const result = GraphDefinitionSchema.safeParse(graphObj);
  if (!result.success) {
    return {
      valid: false,
      errors: (result.error?.issues || []).map((err: { path: (string | number | symbol)[]; message?: string }) => `${err.path.map(p => String(p)).join('.')}: ${err.message || 'Error desconocido'}`),
    };
  }
  ...
  for (const node of validatedGraph.nodes || []) {
    ...
    if (!nodeValidation.valid) {
      ...
      nodeErrors.push(
        ...errors.map((err) => `Nodo ${node.id}: ${err}`)
      );
    }
  }

  // Validar que los edges referencien nodos existentes
  const nodeIds = new Set((validatedGraph.nodes || []).map((n) => n.id));
  const edgeErrors = (validatedGraph.edges || [])
    .filter((e) => !nodeIds.has(e.source) || !nodeIds.has(e.target))
    .map((e) => `Edge ${e.id}: referencia a nodo inexistente`);

  // Validar que start existe
  if (!nodeIds.has(validatedGraph.start)) {
    edgeErrors.push(`Start node '${validatedGraph.start}' no existe en el grafo`);
  }

  // Validar IDs únicos: node.id y edge.id sin duplicados
  const nodeIdSet = new Set<string>();
  const duplicateNodeIds = (validatedGraph.nodes || []).filter((n) => {
  ...
  const duplicateEdgeIds = (validatedGraph.edges || []).filter((e) => {
  ...
  const selfLoops = (validatedGraph.edges || []).filter((e) => e.source === e.target);
  ...
  for (const node of validatedGraph.nodes || []) {
    if (node.type === 'condition.expr' && node.config && typeof node.config === 'object') {
      const config = node.config as { rules?: Array<{ to?: string }> };
      if (config.rules) {
