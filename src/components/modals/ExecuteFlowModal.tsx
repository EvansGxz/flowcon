...
interface ExecuteFlowModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (timeout: number | null) => void;
  defaultTimeout?: number;
}

const ExecuteFlowModal = ({ isOpen, onClose, onConfirm, defaultTimeout = 300 }: ExecuteFlowModalProps) => {
  ...
  const [alertModal, setAlertModal] = useState<{ isOpen: boolean; message: string; type: 'warning' | 'error' | 'info' | 'success' }>({ 
    isOpen: false, 
    message: '', 
    type: 'warning' 
  });
          ...
          <div className="execute-flow-modal-section">
            <label className="execute-flow-modal-label">
              <input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) => setUseCustomTimeout(e.target.checked)}
              />
            ...
            <div className="execute-flow-modal-section">
              <label className="execute-flow-modal-label">
                ...
                <input
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) => handleTimeoutChange(e.target.value)}
