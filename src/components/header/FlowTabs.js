...
const FlowTabs = () => {
  ...
  const handleEditNameChange = (e) => setEditingName(e.target.value);
  ...
  const tabsData = openTabs.map((flowId) => {
    const flow = flows.find((f) => (f.id || f.flow_id) === flowId);
