import { useEffect, useState, useRef, useCallback, FormEvent, ChangeEvent, MouseEvent } from 'react';
...
const ProjectsList = () => {
  ...
  const [contextMenu, setContextMenu] = useState<ContextMenuState>({ isOpen: false, position: null, project: null });
  const [flowsByProject, setFlowsByProject] = useState<Record<string, Flow[]>>({}); // { projectId: [flows] }
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [projectToDelete, setProjectToDelete] = useState<Project | null>(null);

  // Helper para obtener flows de un proyecto específico
  const fetchFlowsForProject = useCallback(async (projectId: string): Promise<Flow[]> => {
    try {
  ...
  const fetchAllFlowsByProject = useCallback(async (projectsList: Project[]): Promise<Record<string, Flow[]>> => {
  ...
  const handleContextMenu = (e: MouseEvent<HTMLDivElement>, project: Project | null = null) => {
    e.preventDefault();
    e.stopPropagation();
    setContextMenu({
      isOpen: true,
  ...
  const handleContextAction = async (action: string, project: Project | null) => {
    switch (action) {
      case 'create':
