...
const ActionNode = ({ data, selected, id }: ActionNodeProps) => {
  ...
  const displayName = data.displayName || definition?.displayName || (data.label as string) || 'Action';
  ...
  const actionType = (config.method as string) || ((data as unknown as { actionType?: string }).actionType) || 'HTTP Request';
