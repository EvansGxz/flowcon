...
const ModelLlmNode = ({ data, selected, id }: ModelLlmNodeProps) => {
  ...
  const temperature = config.temperature !== undefined ? config.temperature : null;
  ...
  if (nodeViewMode === 'icon') {
    return (
      <div
        ref={nodeRef}
        className={`node-container model-llm-node icon-view ${selected ? 'node-selected' : ''} ${statusClass}`}
        style={{ width: '64px', height: '64px', minWidth: '64px' }}
      >
        <div className={`node-indicator node-indicator-violet ${statusClass}`} style={{ width: '100%', height: '100%', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'transparent' }}>
        ...
        <HandleWithClick
          ...
          className="node-handle node-handle-violet"
          nodeId={id}
        />
        <HandleWithClick
          id="out"
  ...
  if (nodeViewMode === 'compact') {
    return (
      <div
        ref={nodeRef}
        className={`node-container model-llm-node compact-view ${selected ? 'node-selected' : ''} ${statusClass}`}
        style={{ minWidth: '200px' }}
      >
        <div className="node-header">
          <div className={`node-indicator node-indicator-violet ${statusClass}`}>
            {status === NodeStatus.RUNNING ? (
  ...
  return (
    <div
      ref={nodeRef}
      className={`node-container model-llm-node informative-view ${selected ? 'node-selected' : ''} ${statusClass}`}
      style={{ minWidth: '200px' }}
    >
      <div className="node-header">
        <div className={`node-indicator node-indicator-violet ${statusClass}`}>
      ...
      {model && (
        <div className="node-badge node-badge-violet">
          Model: {model}
        </div>
      )}
      {provider && (
